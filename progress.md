...
## 2026-07-03 - Task: 恢复报告封面与标题黑色模板格式

### What was done
- 按用户确认的模板要求，将封面主标题、学校名、摘要标题、目录标题和正文标题样式统一为黑色。
- 恢复封面主标题为居中、加粗、大字号展示，保留已完成的自动目录、正文起始页码和实验结果表格内容。
- 本轮仅调整报告视觉格式，不改动实验数据、章节内容和代码逻辑。

### Testing
- DOCX 结构检查通过：封面“自然语言处理”“大作业”为 48 pt、加粗、居中、`000000` 黑色；“山东科技大学”“摘  要”“目  录”为 18 pt、加粗、居中、`000000` 黑色。
- 标题样式检查通过：正文使用的 `heading 1`、`heading 2` 样式颜色均为 `000000` 黑色。
- Word COM 结构验证通过：文档共 2 节，自动目录数量为 1，表格数量为 2，总页数为 10；前置节无页脚页码，正文节页脚页码从 1 开始。
- 文档视觉渲染说明：本机仍缺少 LibreOffice/soffice，未补充 PNG 视觉 QA；本轮使用 DOCX 结构检查与 Word COM 打开检查作为验证证据。

### Notes
- `report/202583230025-刘文羽-中文文本分类课程报告.docx`：恢复封面、摘要、目录和正文标题的黑色模板格式。
- `progress.md`：追加本轮报告标题格式修正日志。
- 回滚方式：提交后可执行 `git revert HEAD` 回滚本轮报告格式和日志修改。
